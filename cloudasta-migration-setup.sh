#!/bin/bash
#
# CloudM Migrate - GCP Service Account & API Setup Script
# =========================================================
# Automates the manual setup steps for CloudM Migrate projects:
#   1. Creates GCP project "Cloudasta-Migration"
#   2. Creates service account "Cloudasta-Migration"
...
#
# Usage:
#   chmod +x cloudasta-migration-setup.sh
#   ./cloudasta-migration-setup.sh
#
# Author: Cloudasta Migration Team
# =========================================================
...
LOG_FILE="${HOME}/cloudasta-migration-${LOG_TIMESTAMP}.log"
...
# COLLECT USER INPUT
# ─────────────────────────────────────────────
print_header "Cloudasta Migration - GCP Setup Script"
...
for api in "${REQUIRED_APIS[@]}"; do
    ...
    if [[ "$API_OK" == true ]]; then
        ...
        FAILED_APIS+=("$api")
        if echo "$API_OUTPUT" | grep -qi "billing"; then
            BILLING_NEEDED=true
...
if [[ -z "$SA_UNIQUE_ID" ]]; then
    print_error "Could not retrieve Service Account Unique ID."
    exit 1
fi
...
if gcloud iam service-accounts keys create "$TEMP_KEY_PATH" \
    ...
    echo ""
    echo -e "${YELLOW}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}║  AUTO-RESOLVE: iam.disableServiceAccountKeyCreation         ║${NC}"
    echo -e "${YELLOW}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    echo ""
    echo -e "${BOLD}Enforced Organization Policy ID:${NC}"
    echo -e "  ${RED}iam.disableServiceAccountKeyCreation${NC}"
    echo ""
    ...
    if [[ "$auto_resolve" == "y" || "$auto_resolve" == "Y" ]]; then
        ...
        if [[ -n "$RESOLVED_ORG_ID" ]]; then
            ...
            # ── Step 3: Disable the iam.disableServiceAccountKeyCreation policy ──
            print_info "Disabling iam.disableServiceAccountKeyCreation policy..."
            POLICY_DISABLED=false

            # Try the newer gcloud org-policies command first (--quiet to avoid interactive API enable prompt)
            if gcloud org-policies reset iam.disableServiceAccountKeyCreation \
                --organization="$RESOLVED_ORG_ID" --quiet 2>&1; then
                POLICY_DISABLED=true
                print_step "Organization policy reset (enforcement OFF)."
            else
                # Fallback: try the older resource-manager command
                print_info "Trying alternative command..."
                if gcloud resource-manager org-policies disable-enforce \
                    iam.disableServiceAccountKeyCreation \
                    --organization="$RESOLVED_ORG_ID" --quiet 2>&1; then
                    POLICY_DISABLED=true
                    print_step "Organization policy enforcement disabled."
                else
                    print_error "Could not disable the organization policy automatically."
                    echo ""
                    ...
                    echo -e "  6. Filter by Policy ID: ${GREEN}iam.disableServiceAccountKeyCreation${NC}"
                    echo -e "  7. Select the Active Policy"
                    echo -e "  8. Click ${BOLD}Manage Policy${NC}"
                    echo -e "  9. Expand the ${BOLD}Enforced${NC} dropdown > Select ${GREEN}OFF${NC} > Click ${BOLD}Done${NC}"
                    echo -e "  10. Click ${BOLD}Set Policy${NC}"
                    echo ""
            ...
            if [[ "$POLICY_DISABLED" == true ]]; then
                ...
                if [[ "$KEY_CREATED" == true ]]; then
                    ...
                    # ── Step 5: Offer to re-enable the policy (security best practice) ──
                    echo ""
                    read -rp "$(echo -e "${YELLOW}Re-enable iam.disableServiceAccountKeyCreation policy for security? (y/n): ${NC}")" reenable_policy
                    if [[ "$reenable_policy" == "y" || "$reenable_policy" == "Y" ]]; then
                        print_info "Re-enabling organization policy..."
                        if gcloud org-policies set-enforce iam.disableServiceAccountKeyCreation \
                            --organization="$RESOLVED_ORG_ID" --quiet 2>&1; then
                            print_step "Organization policy re-enabled."
                        elif gcloud resource-manager org-policies enable-enforce \
                            iam.disableServiceAccountKeyCreation \
                            --organization="$RESOLVED_ORG_ID" --quiet 2>&1; then
                            print_step "Organization policy re-enabled."
                        else
                            print_warn "Could not re-enable automatically."
                            print_info "Remember to re-enable it manually for security:"
                            print_info "IAM & Admin > Organization Policies > iam.disableServiceAccountKeyCreation > Enforce ON"
                        fi
                    else
                        print_warn "Policy remains disabled. Remember to re-enable it after migration for security."
                    fi
                else
...
echo ""
CLOUDASTA_LOGO_URL="https://images.g2crowd.com/uploads/product/image/social_landscape/social_landscape_cee2152a5febe154fc27f9af97b8f3dc/cloudasta-by-shuttlecloud.png"
CHAT_DESCRIPTION="CloudM Migrate"

echo -e "  ${BOLD}App Name:${NC}              ${GREEN}${CHAT_APP_NAME}${NC}"
echo -e "  ${BOLD}Avatar URL:${NC}            ${GREEN}${CLOUDASTA_LOGO_URL}${NC}"
